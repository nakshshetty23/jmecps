        ...
        <div className="flex-grow">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="card-terminal">
              <h3 className="heading-display text-xl text-[#38BDF8] mb-4 border-b border-[#30363D] pb-2">
              ...
              <ul className="space-y-3 font-mono text-sm text-[#F8FAFC] opacity-80">
                {[
                  ...
                  <li key={i} className="flex items-center">
                    <span className="w-1.5 h-1.5 bg-[#EAB308] mr-3"></span>
                    {req}
                  </li>
                ))}
              </ul>
            ...
            <div className="card-terminal">
              <h3 className="heading-display text-xl text-[#EAB308] mb-4 border-b border-[#30363D] pb-2">
                Associate Editors Domains
              </h3>
              <p className="font-mono text-xs text-[#38BDF8] mb-3 uppercase tracking-widest">Experts from different domains:</p>
              <div className="flex flex-wrap gap-2">
                {["Mechanical Engineering", "Electronics Engineering", "AI / ML", "Robotics", "IoT", "Manufacturing", "Control Systems"].map((domain, i) => (
                  <span key={i} className="px-3 py-1 bg-[#0D1117] border border-[#30363D] text-[#F8FAFC] opacity-80 text-xs font-mono uppercase tracking-widest">
                    {domain}
                  </span>
                ))}
              </div>
          ...
          <div className="card-terminal mb-10 border-[#38BDF8]">
            <h3 className="heading-display text-xl text-[#F8FAFC] mb-2">
              International Advisory Board
            </h3>
            <p className="font-mono text-sm text-[#F8FAFC] opacity-80 mb-4">
            ...
            <div className="flex flex-wrap gap-3">
              {["USA", "UK", "Germany", "Canada", "Australia", "Japan", "Singapore", "UAE"].map((country, i) => (
                <div key={i} className="flex items-center px-4 py-2 bg-[#0D1117] border border-[#30363D] font-mono text-sm text-[#F8FAFC] uppercase tracking-widest">
                  <span className="text-[#38BDF8] mr-2">[+]</span> {country}
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-10 mb-8">
            {boardSections.map((section, idx) => (
              <div key={idx}>
                <h2 className="heading-display text-2xl text-[#EAB308] mb-6 flex items-center gap-3 border-b border-[#30363D] pb-3">
                  <span className="w-2 h-6 bg-[#38BDF8]"></span>
                  {section.title}
                </h2>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {section.members.map((member, mIdx) => (
                    <div key={mIdx} className="bg-[#161B22] border border-[#30363D] p-6 hover:border-[#38BDF8] transition-none group flex flex-col h-full">
                      <h3 className="heading-display text-lg text-[#F8FAFC] mb-1 group-hover:text-[#EAB308]">
                        {member.name}
                      </h3>
                      {member.qualification && (
                        <p className="font-mono text-xs text-[#38BDF8] mb-3 uppercase tracking-widest">
                          {member.qualification}
                        </p>
                      )}
                      
                      <div className="space-y-2 mt-auto font-mono text-sm text-[#F8FAFC] opacity-80">
                        <p className="flex items-start gap-2">
                          <span className="text-[#EAB308]">[I]</span>
                          <span>{member.institution}</span>
                        </p>
                        <p className="flex items-start gap-2">
                          <span className="text-[#EAB308]">[L]</span>
                          <span>{member.country}</span>
                        </p>
                        {member.email && (
                          <p className="flex items-start gap-2">
                            <span className="text-[#EAB308]">[C]</span>
                            <a href={`mailto:${member.email}`} className="hover:text-[#38BDF8] transition-none">
                              {member.email}
                            </a>
                          </p>
                        )}
